let fs,chalk;_db2‍.x([["cli",()=>cli]]);_db2‍.w("fs",[["*",null,function(v){fs=v}]]);_db2‍.w("chalk",[["default",["chalk"],function(v){chalk=v}]]);
  ...
  if (operation.type === 'bill') {
    ...
    if (currentUser.canWithdraw &&  currentUser.balance >= Number(operation.amount)) {
      ...
      fs.writeFile(`${__dirname}/store.json`, JSON.stringify(store), function(err) {
        ...
        _db2‍.g.console.log(chalk.greenBright(`Successfully billed ${currentUser.name} ${operation.amount}`));
      })
    } else {
